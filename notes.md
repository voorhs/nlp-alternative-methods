...
Пусть $\vec c(\tau)\in\mathbb{R}^N$ --- вектор коэффициентов полиномиальной проекции функции $f(t)\big|_{[0,\tau]}$. Оказывается, что векторная функция $\vec c(\tau)$ описывается ОДУ:
...
![](figures/rnn-cell-hippo.drawio.svg)

В свое время HiPPO достиг SOTA на ряде задач: на Permuted MNIST измерялась точность классификации изображений как классификация временного ряда; в задаче прогнозировании для хаотической системы Mackey–Glass; на IMDB Sentiment классификация текстов. Ключевые преимущества HiPPO --- быстрый рекуррентный инференс и теоретические гарантии точности аппроксимации. На задачах временных рядов HiPPO показал результаты, сопоставимые не только с RNN, но и с трансформером, превосходя их по скорости инференса в десятки и сотни раз.
...
![center width:1080](figures/s4-clm.png)
