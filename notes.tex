...
% meta
\title{Альтернативные методы NLP}
\author{\href{https://github.com/voorhs/nlp-alternative-methods}{Contribute on GitHub}}
\date{Ноябрь 2025}
...
\begin{titlepage}
    \begin{center}
        {\large Московский государственный университет имени М.В. Ломоносова}\\[0.2em]
        {\large Факультет вычислительной математики и кибернетики}

        \vspace{6cm}

        {\Large Реферат}\\[1em]
        {\Large \textbf{Альтернативные методы NLP}}
    \end{center}
...
\begin{abstract}
Данная лекция представляет всесторонний обзор State-Space Models (SSMs), прослеживая их эволюцию от фреймворка HiPPO~\cite{gu2020hipporecurrentmemoryoptimal} до современной архитектуры Mamba~\cite{gu2024mambalineartimesequencemodeling}, а также расширяет обсуждение на родственные линейные архитектуры. Мы начинаем с математических основ (полиномиальные проекции, дифференциальные уравнения) и показываем, как они воплощаются в трёх направлениях развития: рекуррентная память HiPPO, структурированные и селективные SSM-слои (LSSL~\cite{gu2021combiningrecurrentconvolutionalcontinuoustime}, S4~\cite{gu2022efficientlymodelinglongsequences}, DSS~\cite{gupta2022diagonalstatespaceseffective}, S4D~\cite{gu2022parameterizationinitializationdiagonalstate}, Mamba). Далее рассматриваются линейные трансформеры~\cite{katharopoulos2020transformersrnnsfastautoregressive,yang2025parallelizinglineartransformersdelta,kimiteam2025kimilinearexpressiveefficient}, демонстрирующие, как идеи внимания переносятся на рекуррентные архитектуры, и сопоставляются с SSM через призму дуальности трансформеров и моделей состояний~\cite{dao2024transformersssmsgeneralizedmodels}. Финальный блок посвящён RWKV~\cite{peng2023rwkvreinventingrnnstransformer,rwkv}. На протяжении всего текста подчёркиваются теоретические связи, инженерные компромиссы и практические реализации, позволяющие достигать линейной временной сложности без потери качества на задачах языкового моделирования.
\end{abstract}

\section{State-space models}

\subsection{Математические предпосылки}
...
\subsection{HiPPO: Рекуррентная память с оптимальными полиномиальными проекциями}
...
\subsubsection{Реализации HiPPO}
...
\subsubsection{Linear State-Space Layer (LSSL)}
...
\subsubsection{Стратегии параметризации}
...
\subsection{Mamba: Селективные модели пространства состояний}
...
\subsubsection{Linear Attention}\label{sec:linear_att}
...
\subsubsection{Hungry Hungry Hippos (H3)}
...
\subsubsection{Селективные модели пространства состояний}
...
\subsubsection{Архитектура Mamba}
...
\subsubsection{Сравнение с трансформерами}
...
В настоящий материал не включен рассказ про Mamba-2 \cite{dao2024transformersssmsgeneralizedmodels} и Mamba-3 \cite{anonymous2025mamba}.

\section{Линейные трансформеры}

\subsection{Общие предпосылки}

Одним из многообещающих направлений развития альтернативных подходов являются так называемые линейные трансформеры. Вспомним идею линейного внимания, описанную в разделе \ref{sec:linear_att}. Если в формуле механизма внимания заменить $\text{sim}(q,k)=\text{exp}(q^Tk)$ на $\text{sim}(q,k)=\phi(q)^T\phi(k)$, то получаем рекуррентный инференс (линейный по времени относительно длины последовательности):
\begin{align*}
  S_i&=S_{i-1}+\phi(K_i)V_i^T,\\
  Z_i&=Z_{i-1}+\phi(K_i).
\end{align*}

Эта идея развивалась, и сегодня мы имеем целый ряд LLM архитектур, основанных на линейном аттеншене \cite{yang2025parallelizinglineartransformersdelta, kimiteam2025kimilinearexpressiveefficient, qwen32next}. В целях краткости рассказа перечислим особенности этих архитектур тремя ключевыми апгрейдами.

\textbf{Апгрейд 0: Без нелинейности.} Во-первых, в формулах линейного трансформера можно убрать функцию $\phi(\cdot)$ (TODO cide):
\begin{align*}
  S_i&=S_{i-1}+K_iV_i^T,\\
  Z_i&=Z_{i-1}+K_i.
\end{align*}
Более того, некоторые работы показывают, что от знаменателя $Z_i$ тоже можно избавиться, поскольку он вносит численную нестабильность. Тогда инференс станет линейным в смысле преобразований, а не только всмысле времени. Обоснование у такого подхода в первую эмпирическое. Однако полученную формулу $S_i=S_{i-1}+K_iV_i^T$ можно обосновать еще теоретически. Смысл в том, что она эквивалентна градиентной оптимизации следующего функционала:
\begin{equation*}
\mathcal{L}_i=-\langle SK_i,V_i \rangle.
\end{equation*}
То есть можно сказать, что в ходе рекуррентного инференса мы используем память $S$ как некоторый линейный оператор для предсказания $V_i$ с помощью $K_i$. Обучение такого оператора происходит засчет минимизации скалярного произведения между предсказанием и таргетом.

\textbf{Апгрейд 1: Delta-правило.} Вместо $\mathcal{L}_i=-\langle SK_i,V_i \rangle$ можно использовать MSE-лосс:
\begin{equation*}
  \mathcal{L}_i=-{1\over 2}\|SK_i-V_i\|^2.
\end{equation*}
Тогда оптимизация будет выглядеть следующим образом:
\begin{align*}
S_i &= S_{i-1} - \beta_i\nabla\mathcal{L}_{i-1}\\
&= S_{i-1}− \beta_i(S_{i-1}K_i− V_i)K_i
\end{align*}
Поскольку здесь возникает разница между предсказанием и таргетом, эту формулу называют delta-rule. В качестве $\beta_i$ берут $\beta_i=\sigma(W_\beta X_i)$ --- обучаемую нелинейную функцию (с сигмоидой). Это правило интерпретируют как более точное обновление рекуррентной памяти $S$. Более того, существуют эффективные hardware-aware алгоритмы, которые позволяют параллелить обучение по этим формулам \cite{yang2025parallelizinglineartransformersdelta}.

\textbf{Апгрейд 2: Gated attention}. В архитектуру современных трансформеров все чаще добавляют так называемый gated attention вместо обычного механизма внимания \cite{qiu2025gatedattentionlargelanguage}. Прием заключается в добавлении специальной нелинейности после обычного атеншена перед конкатенацией выходов с разных голов (рис. \ref{fig:gated-attention}). Необходимость в этой дополнительной нелинейности преимущественно объясняется эффектом под названием attention sink --- когда внимание на начальные токены последовательности доминируют для всех токенов последовательности, как далеко от начала они бы не находились (рис. \ref{fig:attention-sink}).


\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/gated-attention.png}
  \caption{Результаты основного эксперимента из статьи Gated Attention \cite{qiu2025gatedattentionlargelanguage}.}
  \label{fig:gated-attention}
\end{figure}

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/attention-sink.png}
  \caption{Эффект attention sink из статьи Gated Attention \cite{qiu2025gatedattentionlargelanguage}.}
  \label{fig:attention-sink}
\end{figure}

\subsection{Примеры}

\textbf{Gated DeltaNet} \cite{yang2025gateddeltanetworksimproving}. Это одна из первых работ, соединивших все перечисленные апгрейды. Дополнительно авторы ввели скалярный forget gate для рекуррентной памяти:
$$
S_i = \alpha_i(I−\beta_i K_i K_i^T)S_{i−1} + \beta_i K_i V_i^T.
$$ 

\textbf{Kimi Linear} \cite{kimiteam2025kimilinearexpressiveefficient}. Во-первых, в этой работе скалярный $\alpha_i$ заменили на диагональную матрицу $\diag(\vec\alpha_i)$:
$$
S_i = I−\beta_i K_i K_i^T\diag (\vec\alpha_i)S_{i−1} + \beta_i K_i V_i^T.
$$
Во-вторых, авторы детальнее проработали архитектуру всей модели, добавив свёртки и RMSProp-нормирование голов (рис. \ref{fig:kimi-arch}).

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/kimi-arch.png}
  \caption{Архитектура Kimi Linear \cite{kimiteam2025kimilinearexpressiveefficient}.}
  \label{fig:kimi-arch}
\end{figure}

\textbf{Qwen-3.2-next} \cite{qwen32next}. Эта модель очень похожа на Kimi Linear и по большей части отличается лишь тем, что не целиком отказывается от стандартного внимания взамен на линейное, а комбинирует оба подхода, образуя гибридную архитектуру (рис. \ref{fig:qwen-arch}).

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/qwen-arch.png}
  \caption{Архитектура Qwen-3.2-next \cite{qwen32next}.}
  \label{fig:qwen-arch}
\end{figure}

\section{RWKV}

Название RWKV принадлежит целой серии моделей, которая сегодня насчитывает семь последовательно выпускавшихся версий \cite{rwkv}. Ниже поговорим о четвертой версии --- первой, о которой вышла статья \cite{peng2023rwkvreinventingrnnstransformer}.

Во-первых, авторы используют измененную функцию близости между ключом и запросом: вместо $\text{sim}(q_i,k_j)=\text{exp}(q_i^Tk_j)$ считают $\text{sim}(q_i,k_j)=\exp(k_i-(i-j)w)$, где $w\in\mathbb{R}^d$ --- обучаемый вектор. Интуиция такова, что чем дальше друг от друга ключ и запрос, тем менее они релевантны друг другу. Вместо вычисления сложных семантических отношений между ними с помощью скалярного произведения, достаточно просто знать, как далеко они друг от друга. Это сильное упрощение, но вместе с другими аспектами RWKV оно не сильно портит качество. При этом за счет избавления от $q$ матрицу внимания легко считать на каждом новом шаге рекуррентного инференса.

Во-вторых, авторы используют архитектуру, имитирующую механизм внимания (рис. \ref{fig:rwkv-arch}). Вектор $R$ на этой схеме и в названии RWKV означает receptance и он заменяет векторы $Q_i$. Заметим, что в этой архитектуре предыдущие векторы не выкидываются, а хранятся, поскольку они используются в WKV операторе. Главный плюс в том, что благодаря упрощенной функции близости $\text{sim}(q_i,k_j)=\exp(k_i-(i-j)w)$ получается сохранить параллельность обучения и достичь линейный инференс.

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.6\textwidth]{figures/rwkv.png}
  \caption{Архитектура RWKV-v4 \cite{peng2023rwkvreinventingrnnstransformer}. На этом рисунке WKV означает формулу внимания, в которой $\text{sim}(q_i,k_j)=\exp(k_i-(i-j)w)$.}
  \label{fig:rwkv-arch}
\end{figure}

В-третьих, помимо рекуррентной связи состояний в RWKV используется сглаженное среднее входов ($\mu$ на схеме \ref{fig:rwkv-arch}), которое авторы называют token shift:
\begin{align*}
r_i &= W_r(\mu_r \cdot x_i + (1−\nu_r)\cdot x_i−1),\\
k_i &= W_k(\mu_k \cdot x_i + (1−\nu_k)\cdot x_i−1),\\
v_i &= W_v(\mu_v \cdot x_i + (1−\nu_v)\cdot x_i−1)
\end{align*}

В-четвертых, для стабилизации и ускорения обучения RWKV пришлось применить несколько маленьких оптимизаций:
\begin{enumerate}
  \item кастомное CUDA-ядро для оператора WKV
  \item специальная инициализация матрицы эмбедингов токенов
  \item специальная инициализация весов сети
\end{enumerate}
