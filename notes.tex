...
До Mamba, Linear Attention показал, как эффективно аппроксимировать внимание трансформера:
...
Поскольку свёртка в явном виде недоступна, для распараллеливания рекуррентности используется алгоритм parallel scan, который обычно в алгоритмах используют для вычисления кумулятивной суммы.
...
\caption{Результаты основного эксперимента из оригинальной статьи Mamba. Оригинальную статью Mamba реджектнули с ICML 2024. Сможете ли вы предположить, почему? Комментарии рувьюеров тут: \url{https://openreview.net/forum?id=AL1fq05o7H}}
